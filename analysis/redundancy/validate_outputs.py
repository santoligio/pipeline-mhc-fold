...
def validate(output_dir: Path) -> dict:
    ...
    for group, rows in reps_by_group.items():
        ...
        assert representative == min(members, key=lambda pdb_id: rep_key(catalog_by_id[pdb_id])), (
            f"{group}: representative violates MHC-length/resolution/binder-length/PDB ordering"
        )
